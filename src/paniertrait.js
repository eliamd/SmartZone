...
function displaycart() {
  ...
  if (cartitems && prdcon) {
    ...
    Object.values(cartitems).map((item) => {
      prdcon.innerHTML += `
        <div id='${
          item.id
        }' class='product flex items-center hover:bg-gray-100 -mx-8 px-6 py-5'>
          ...
          <div class='flex justify-center w-1/5'>
            ...
            <span class='mx-2 border text-center w-10 px-0 py-2 rounded-sm bg-white'>${
              item.incart
            }</span>
  ...
  if (nbitems != 0) {
    document.getElementById("orbtn").className =
      "bg-orange-500 font-semibold hover:bg-orange-600 py-3 text-sm text-white uppercase w-full rounded-md";
  } else {
    document.getElementById("orbtn").className =
      "bg-orange-400 font-semibold py-3 text-sm text-white uppercase w-full rounded-md cursor-not-allowed";
  }
...
function deleteButtons() {
  ...
  for (let i = 0; i < deleteButtons.length; i++) {
    deleteButtons[i].addEventListener("click", () => {
      ...
      localStorage.setItem(
        "totalcost",
        cartCost - cartItems[productName].prix * cartItems[productName].incart
      );
